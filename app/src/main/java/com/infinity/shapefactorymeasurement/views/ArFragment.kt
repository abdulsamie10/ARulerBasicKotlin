...
class ArFragment: Fragment(R.layout.fragment_ar) {
    ...
    private fun removeLastNode() {
        if (currentAnchorNode.isNotEmpty()) {
            val lastAnchorNode = currentAnchorNode.last()
            val lastAnchor = currentAnchor.last()
            lastAnchor?.detach()
            lastAnchorNode.setParent(null)
            currentAnchorNode.remove(lastAnchorNode)
            currentAnchor.remove(lastAnchor)
            labelArray.removeLast()

            if (currentAnchorNode.size >= 2) {
                val previousNode = currentAnchorNode[currentAnchorNode.size - 1]
                val secondLastNode = currentAnchorNode[currentAnchorNode.size - 2]
                val node1Pos = previousNode.worldPosition
                val node2Pos = secondLastNode.worldPosition
            ...
            if (::lineBetween.isInitialized) {
                lineBetween.anchor?.detach()
                lineBetween.setParent(null)
                arFragment.arSceneView.scene.removeChild(lineBetween)
            }

            lengthLabel?.anchor?.detach()
            lengthLabel?.setParent(null)
            lengthLabel = null
        }
    }
    ...
    private fun resetARSession() {
        arFragment.arSceneView.session?.let { session ->
            ...
            for (anchorNode in currentAnchorNode) {
            ...
            // Reset temporary anchor and renderables
            anchorNodeTemp?.anchor?.detach()
            anchorNodeTemp?.setParent(null)
            anchorNodeTemp = null

            // Reset lineBetween and lengthLabel
            if (::lineBetween.isInitialized) {
                lineBetween.anchor?.detach()
                lineBetween.setParent(null)
            }
